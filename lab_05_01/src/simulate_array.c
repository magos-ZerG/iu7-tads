...
int simulate_arr(int verbose)
{
    ...
    for (size_t i = 0; i < repeats_num; i++)
    {
        ...
        while (req1_count < MAX_REQ_COUNT)
        {
            ...
            {
                tmp_t = gen_T2();
                rc = push_back_arr(&arr_queue, tmp_t);
                if (rc != QUEUE_OK)
                    return rc;
                req1_timeout = gen_T1();
            }
            ...
            while (handler == 0)
            {
                if (req2_num_timeout >= 4 || arr_queue.queue_size == 0)
                {
                    handler = gen_T3();
                    req2_num_timeout = 0;
                    req2_count++;
            ...
            if (handler > 0)
                handler--;
            avg_queue_len += arr_queue.queue_size;
        }
